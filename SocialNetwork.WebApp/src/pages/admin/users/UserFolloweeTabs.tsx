...
type UserFolloweeTabsProps = {
    userId: string;
}

const UserFolloweeTabs: FC<UserFolloweeTabsProps> = ({
    userId
}) => {
    ...
    const fetchFollowees = async (page: number, size: number) => {
        setLoading(true)
        const response = await adminService.getAllFolloweesByUserId(userId, page, size, searchValue);
        setLoading(false)
    ...
    return <div>
        <div className="lg:max-w-screen-lg md:max-w-screen-md max-w-screen-sm mx-auto px-4 w-full py-3 flex flex-col gap-y-3">
            <div className="flex items-center gap-x-2">
                <span className="text-xl font-bold">Tổng số người đang theo dõi</span>
                <span className="w-1 h-1 rounded-full bg-gray-500"></span>
                <span className="text-xl font-bold">{followees.length}</span>
            </div>
        ...
        <Table
            ...
            onChange={(paginate) => {
                if (paginate.current) {
                    fetchFollowees(paginate.current, pagination.size)
                }
            }}
        />
    </div>
...
export default UserFolloweeTabs;
