...
export type ModifyUserLocationRequest = {
    address: string;
    locationId: string | null;
}


type ModifyUserLocationProps = {
    userLocation: LocationResource | undefined;
    onFetch?: () => void
}

const ModifyUserLocation: FC<ModifyUserLocationProps> = ({
    userLocation,
    onFetch
}) => {
    const [form] = useForm<ModifyUserLocationRequest>();
    const [searchValue, setSearchValue] = useState<string>('');

    const [locations, setLocations] = useState<LocationResource[]>([])
    const [disabled, setDisabled] = useState(true)

    const onFinish: FormProps<ModifyUserLocationRequest>['onFinish'] = async (values) => {
        const response = await userService.modifyUserLocation(values);
    ...
    return <div className="max-h-[550px] overflow-y-auto custom-scrollbar">
        <Form
            ...
            layout="vertical"
        >
            <Form.Item<ModifyUserLocationRequest>
                label="Địa điểm"
                name="address"
                rules={[{ required: true, message: 'Vui lòng nhập địa điểm!' }]}
            >
                <InputSearchDropdown
                    id={`address-${userLocation?.id}`}
