    ...
    return <div onClick={onClick} className={cn("cursor-pointer flex items-center gap-x-3 px-3 py-3 rounded-sm w-full hover:bg-gray-100", isActive && 'bg-gray-100')}>
        ...
        <div className="flex flex-col overflow-hidden w-full">
            <div className="flex items-center justify-between">
                ...
                <span className={cn("text-[11px]", chatRoom.isRead ? 'text-gray-400' : 'text-primary font-semibold')}>{formatTime(new Date(chatRoom.lastMessageDate))}</span>
            </div>
            <p className={cn("truncate text-xs", chatRoom.isRead ? 'text-gray-400' : 'font-semibold text-black')}>{chatRoom.lastMessage}</p>
        </div>
