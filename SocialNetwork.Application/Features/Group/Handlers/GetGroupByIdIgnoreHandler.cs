...
{
    public class GetGroupByIdIgnoreHandler : IRequestHandler<GetGroupByIdIgnoreQuery, BaseResponse>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IHttpContextAccessor _contextAccessor;

        public GetGroupByIdIgnoreHandler(IUnitOfWork unitOfWork, IHttpContextAccessor contextAccessor) {
            _unitOfWork = unitOfWork;
            _contextAccessor = contextAccessor;
        }
        public async Task<BaseResponse> Handle(GetGroupByIdIgnoreQuery request, CancellationToken cancellationToken)
        {
            var group = await _unitOfWork.GroupRepository.GetGroupByIdIgnoreAsync(request.GroupId)
