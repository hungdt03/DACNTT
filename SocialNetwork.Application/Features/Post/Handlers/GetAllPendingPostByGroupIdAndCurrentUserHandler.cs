...
{
    public class GetAllPendingPostByGroupIdAndCurrentUserHandler : IRequestHandler<GetAllPendingPostByGroupIdAndCurrentUserQuery, BaseResponse>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IHttpContextAccessor _contextAccessor;

        public GetAllPendingPostByGroupIdAndCurrentUserHandler(IUnitOfWork unitOfWork, IHttpContextAccessor contextAccessor)
        {
            _unitOfWork = unitOfWork;
            _contextAccessor = contextAccessor;
        }

        public async Task<BaseResponse> Handle(GetAllPendingPostByGroupIdAndCurrentUserQuery request, CancellationToken cancellationToken)
        {
            var userId = _contextAccessor.HttpContext.User.GetUserId();

            var (posts, totalCount) = await _unitOfWork.PostRepository
                .GetAllGroupPostsByGroupIdAndUserIdAndStatus(request.GroupId, userId, ApprovalStatus.PENDING, request.Page, request.Size);
            ...
            return new PaginationResponse<List<PostResponse>>()
            {
                IsSuccess = true,
                Data = response,
                Message = "Lấy danh sách bài viết đang chờ thành công",
